...
class TestGenerateGalleryHTML:
    ...
    def test_generate_html_images_copied(self, sample_config, sample_images):
        """Test that original images are copied to originals directory.

        Note: Thumbnails are generated by build_gallery(), not generate_gallery_html().
        This unit test focuses on the HTML generation function's behavior.
        """
        images = [
            Image(
                filename=f.name,
        ...
        # Check images directory structure for originals
        ...
        # Check original images were copied with hashing
        copied_images = list(originals_dir.glob("*.jpg"))
